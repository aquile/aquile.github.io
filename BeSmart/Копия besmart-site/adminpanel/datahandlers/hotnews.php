...
function getEntityList($WHERE, $start = null, $limit = null) {
    $result = array();
    $q = 'SELECT SQL_CALC_FOUND_ROWS * FROM `' . cnf::$db_prefix . 'hotnews`';
    $q_where = '';
...
function updateEntity($info) {
    ...
    if ($info && count($info)) {
        $q = 'UPDATE `' . cnf::$db_prefix . 'hotnews` SET ';
        $q_sub = '';
...
function insertNewEntity($info) {
    $q = sprintf('INSERT INTO `' . cnf::$db_prefix . 'hotnews`(`id`, `title`, `content`, `status`) VALUES(0, "%s", "%s", %d)', 
            mysql_real_escape_string($info['title']),
            mysql_real_escape_string($info['content']),
...
function deleteEntity($id) {
    $q = sprintf('DELETE FROM `' . cnf::$db_prefix . 'hotnews` WHERE `id` = %d', $id);
    mysql_query($q) or die(mysql_error() . $q);
}
?>