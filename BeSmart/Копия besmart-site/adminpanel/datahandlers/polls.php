...
function getEntityList($WHERE, $start = null, $limit = null) {
    $result = array();
    $q = 'SELECT SQL_CALC_FOUND_ROWS * FROM `' . cnf::$db_prefix . 'polls`';

    $q_where = ' WHERE `department` = ' . (SystemSettings::getByName('DEPARTMENT') ? SystemSettings::getByName('DEPARTMENT') : 1);
    ...
    while($row = mysql_fetch_assoc($res)) {
        $q_res = mysql_query('SELECT * FROM `' . cnf::$db_prefix . 'polls_results` WHERE `question`=' . $row['id']);
        ...
        while ($row_res = mysql_fetch_assoc($q_res)) {
            $row['results'][$row_res['answer']]['answers'] ++;
        }
...
function updateEntity($info) {
    ...
    if ($info && count($info)) {
        $q = 'UPDATE `' . cnf::$db_prefix . 'polls` SET ';
        $q_sub = '';
...
function insertNewEntity($info) {
    $q = sprintf('INSERT INTO `' . cnf::$db_prefix . 'polls`(`id`, `question`, `options`, `active`, `department`) VALUES(0, "%s", "%s", %d, %d)', 
            mysql_real_escape_string($info['question']),
            mysql_real_escape_string($info['options']),
            $info['active'] ? 1 : 0,
...
function deleteEntity($id) {
    $q = sprintf('DELETE FROM `' . cnf::$db_prefix . 'polls` WHERE `id` = %d', $id);
    mysql_query($q) or die(mysql_error() . $q);
}
?>