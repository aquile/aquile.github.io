...
function getEntityList($WHERE, $start = null, $limit = null) {
    $result = array();
    $q = 'SELECT SQL_CALC_FOUND_ROWS * FROM `' . cnf::$db_prefix . 'levels`';
    $q_where = '';
...
function updateEntity($info) {
    ...
    if ($info && count($info)) {
        $q = 'UPDATE `' . cnf::$db_prefix . 'levels` SET ';
        $q_sub = '';
...
function insertNewEntity($info) {
    $q = sprintf('INSERT INTO `' . cnf::$db_prefix . 'levels`(`id`, `name`) VALUES(0, "%s", "%s")', 
...
function deleteEntity($id) {
    $q = sprintf('DELETE FROM `' . cnf::$db_prefix . 'levels` WHERE `id` = %d', $id);
    mysql_query($q) or die(mysql_error() . $q);
}
?>