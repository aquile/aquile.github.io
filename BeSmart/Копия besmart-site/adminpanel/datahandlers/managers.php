...
function getEntityList($WHERE, $start = null, $limit = null) {
    $result = array();
    $q = 'SELECT SQL_CALC_FOUND_ROWS managers.* FROM `' . cnf::$db_prefix . 'managers`'
            . ' LEFT OUTER JOIN `' . cnf::$db_prefix . 'schools` ON `schools`.id = `managers`.`school`';
    $q_where = ' WHERE managers.deleted = 0 AND `schools`.`department` = ' . (SystemSettings::getByName('DEPARTMENT') ? SystemSettings::getByName('DEPARTMENT') : 1);
...
function updateEntity($info) {
    ...
    if ($info && count($info)) {
        $q = 'UPDATE `' . cnf::$db_prefix . 'managers` SET ';
        $q_sub = '';
...
function insertNewEntity($info) {
    $q = sprintf('INSERT INTO `' . cnf::$db_prefix . 'managers`(`name`, `tel`, `email`, `school`) VALUES("%s", "%s", "%s", %d)', 
...
function deleteEntity($id) {
    $q = sprintf('UPDATE `' . cnf::$db_prefix . 'managers` SET `deleted` = 1 WHERE `id` = %d', $id);
    mysql_query($q) or die(mysql_error() . $q);
}