...
function getEntityList($WHERE, $start = null, $limit = null) {
    $result = array();
    $q = 'SELECT SQL_CALC_FOUND_ROWS * FROM `' . cnf::$db_prefix . 'schools`';
...
function updateEntity($info) {
    ...
    if ($info && count($info)) {
        $q = 'UPDATE `' . cnf::$db_prefix . 'schools` SET ';
        $q_sub = '';
...
function insertNewEntity($info) {
    $q = sprintf('INSERT INTO `' . cnf::$db_prefix . 'schools`(`id`, `name`, `address`) VALUES(0, "%s", "%s")', 
            mysql_real_escape_string($info['name']),
            mysql_real_escape_string($info['address']));
...
function deleteEntity($id) {
    $q = sprintf('DELETE FROM `' . cnf::$db_prefix . 'schools` WHERE `id` = %d', $id);
    mysql_query($q) or die(mysql_error() . $q);
}
