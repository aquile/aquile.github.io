...
// VIEW request
    $pages = getLessonTypes(array(), $_POST['start'], $_POST['limit']);
...
function getLessonTypes($WHERE, $start = null, $limit = null) {
    $result = array();
    $q = 'SELECT SQL_CALC_FOUND_ROWS * FROM `' . cnf::$db_prefix . 'userlessonstatus`';
    $q_where = ' WHERE id != 7 ';
...
function updateLessonType($info) {
    ...
    if ($info && count($info)) {
        $q = 'UPDATE `' . cnf::$db_prefix . 'userlessonstatus` SET ';
        $q_sub = '';
    ...
    return getLessonTypes(array('id' => $id), 0, 1);
}

function insertNewLessonType($info) {
    $q = sprintf('INSERT INTO `' . cnf::$db_prefix . 'userlessonstatus`(`name`, `color`) VALUES("%s", "%s")', 
            mysql_real_escape_string($info['name']),
            mysql_real_escape_string($info['color']));
...
function deleteLessonType($id) {
    $q = sprintf('DELETE FROM `' . cnf::$db_prefix . 'userlessonstatus` WHERE `id` = %d', $id);
    mysql_query($q) or die(mysql_error() . $q);
}
?>