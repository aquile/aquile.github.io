...
function getEntityList($WHERE, $start = null, $limit = null) {
    $result = array();
    $q = 'SELECT SQL_CALC_FOUND_ROWS * FROM `' . cnf::$db_prefix . 'tests`';
    $q_where = '';
...
function updateEntity($info) {
    ...
    if ($info && count($info)) {
        $q = 'UPDATE `' . cnf::$db_prefix . 'tests` SET ';
        $q_sub = '';
...
function insertNewEntity($info) {
    $q = sprintf('INSERT INTO `' . cnf::$db_prefix . 'tests`(`name`,`level`,`type`,`successRate`, `timeLimit`) VALUES("%s", %d, %d, "%s", "%s")', 
            mysql_real_escape_string($info['name']),
            mysql_real_escape_string($info['level']),
            mysql_real_escape_string($info['type']),
            mysql_real_escape_string($info['successRate']),
            mysql_real_escape_string($info['timeLimit']));
...
function deleteEntity($id) {
    $q = sprintf('DELETE FROM `' . cnf::$db_prefix . 'tests` WHERE `id` = %d', $id);
    mysql_query($q) or die(mysql_error() . $q);
}
?>