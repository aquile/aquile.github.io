...
function getEntityList($WHERE, $start = null, $limit = null) {
    $result = array();
    $q = 'SELECT SQL_CALC_FOUND_ROWS * FROM `' . cnf::$db_prefix . 'packages`';
...
function updateEntity($info) {
    ...
    if ($info && count($info)) {
        $q = 'UPDATE `' . cnf::$db_prefix . 'packages` SET ';
        $q_sub = '';
...
function insertNewEntity($info) {
    $q = sprintf('INSERT INTO `' . cnf::$db_prefix . 'packages`(`name`, `days`, `day_price`, `department`) VALUES("%s", %d, %d, %d)', 
            mysql_real_escape_string($info['name']),
            mysql_real_escape_string($info['days']),
            mysql_real_escape_string($info['day_price']),
...
function deleteEntity($id) {
    $q = sprintf('DELETE FROM `' . cnf::$db_prefix . 'packages` WHERE `id` = %d', $id);
    mysql_query($q) or die(mysql_error() . $q);
}
?>