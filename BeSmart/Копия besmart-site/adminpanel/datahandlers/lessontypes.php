...
function getLessonTypes($WHERE, $start = null, $limit = null) {
    $result = array();
    $q = 'SELECT SQL_CALC_FOUND_ROWS * FROM `' . cnf::$db_prefix . 'lesson_types`';
    $q_where = '';
...
function updateLessonType($info) {
    ...
    if ($info && count($info)) {
        $q = 'UPDATE `' . cnf::$db_prefix . 'lesson_types` SET ';
        $q_sub = '';
...
function insertNewLessonType($info) {
    $q = sprintf('INSERT INTO `' . cnf::$db_prefix . 'lesson_types`(`id`, `name`) VALUES(0, "%s")', 
...
function deleteLessonType($id) {
    $q = sprintf('DELETE FROM `' . cnf::$db_prefix . 'lesson_types` WHERE `id` = %d', $id);
    mysql_query($q) or die(mysql_error() . $q);
}
?>