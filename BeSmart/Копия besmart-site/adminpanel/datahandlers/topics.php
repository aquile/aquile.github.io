...
function getEntityList($WHERE, $start = null, $limit = null) {
    $result = array();
    $q = 'SELECT SQL_CALC_FOUND_ROWS * FROM `' . cnf::$db_prefix . 'lesson_topics`';
...
function updateEntity($info) {
    ...
    if ($info && count($info)) {
        $q = 'UPDATE `' . cnf::$db_prefix . 'lesson_topics` SET ';
        $q_sub = '';
...
function insertNewEntity($info) {
    $q = sprintf('INSERT INTO `' . cnf::$db_prefix . 'lesson_topics`(`name`,`levelID`,`lessontypeID`,`grammar`) VALUES("%s", %d, %d, "%s")', 
            mysql_real_escape_string($info['name']),
            mysql_real_escape_string($info['levelID']),
            mysql_real_escape_string($info['lessontypeID']),
            mysql_real_escape_string($info['grammar']));
...
function deleteEntity($id) {
    $q = sprintf('UPDATE `' . cnf::$db_prefix . 'lesson_topics` SET `deleted`=1 WHERE `id` = %d', $id);
    mysql_query($q) or die(mysql_error() . $q);
}
?>